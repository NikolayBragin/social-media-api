...
from database.postservice import add_new_post_db, delete_post_db, edit_post_text_db, get_exacr_post_db, get_all_posts_db, like_post_db, unlike_post_db


# Запрос на вывод всех постов
@posts_router.get('/all-posts')
async def all_posts_info():
...
async def get_exact_post(post_id: int):
    result = get_exacr_post_db(post_id)
...
async def like_post(post_id: int):
    result = like_post_db(post_id=post_id)
...
async def unlike_post(post_id: int):
    result = unlike_post_db(post_id=post_id)
